...
# SBERT (Semantic power)
print("Encoding with SBERT...")
model_sbert = SentenceTransformer('all-MiniLM-L6-v2')
resume_embeddings = model_sbert.encode(df['all_resume_text'].tolist(), convert_to_tensor=True)
job_embeddings = model_sbert.encode(df['all_job_text'].tolist(), convert_to_tensor=True)
...
features = ['tfidf_sim', 'semantic_sim', 'skill_match_ratio', 'title_match_feature', 'field_match_feature', 'age', 'experiencere', 'exp_diff', 'age_diff']
...
print(f"HYBRID SBERT+TFIDF RESULTS -> R2: {r2_score(y_test, y_pred):.4f}, MAE: {mean_absolute_error(y_test, y_pred):.4f}")
